...
class TestLoadPresets(unittest.TestCase):
    def test_no_path_means_no_presets(self):
        self.assertEqual(subs2srt.load_presets(None), {})
        self.assertEqual(subs2srt.load_presets(""), {})

    def test_missing_file_means_no_presets(self):
        self.assertEqual(subs2srt.load_presets("/no/such/presets.json"), {})

    def test_engine_package_ships_no_preset_file(self):
        engine_dir = os.path.dirname(subs2srt.__file__)
...
class TestNewsPresets(unittest.TestCase):
    ...
    def setUp(self):
        self.presets = subs2srt.load_presets(paths.ENGINE_PRESETS)
    ...
    def test_mxf_batch_still_matches_by_file_name(self):
        key, _ = subs2srt.match_preset("20NL003_32午間族語新聞.mxf",
