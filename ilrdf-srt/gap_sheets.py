...
def prepare(slug, wenkao):
    ...
    if wenkao and os.path.isdir(wenkao):
        records, _, _ = aligner.resolve(src, wenkao)
        for rec in records:
    ...
    with open(os.path.join(dst, "from_wenkao.json"), "w",
              encoding="utf-8") as handle:
...
def main():
    ...
    for entry in entries:
        ...
        if not os.path.exists(os.path.join(WORK, slug + ".work", "cues.json")):
            ...
            continue
        dst = os.path.join(WORK, slug + ".B.work")
        if os.path.exists(os.path.join(dst, "from_wenkao.json")) \
                or already_read(dst):
            print("skip %s (already prepared or read)" % slug)
            continue
        wenkao = (os.path.join(CORPUS, entry["文稿位置"])
                  if entry["文稿位置"] else "")
        cues, aligned, gap, made = prepare(slug, wenkao)
        total_sheets += made
