...
CORPUS = "/home/vscode/ilrdf-corpus"
WORK = os.path.join(ROOT, "kithann/out/mxf")
SRT_DIR = os.path.join(ROOT, "kithann/srt")
WENKAO_DIR = os.path.join(SRT_DIR, "wenkao-only")
...
def make_one(entry):
    ...
    if vision_complete(vision):
        ...
        proc = subprocess.run(
            [PY, os.path.join(HERE, "make_srt.py"), vision, "-o", out],
            capture_output=True, text=True)
        if proc.returncode != 0:
            return "錯誤：%s" % (proc.stderr.strip().splitlines()
                               or ["?"])[-1][:80]
    ...
    wenkao = ""
    if entry["文稿位置"]:
        wenkao = os.path.join(CORPUS, entry["文稿位置"])

    name = entry["srt_name"]
    out = os.path.join(SRT_DIR, name + ".srt")
    wout = os.path.join(WENKAO_DIR, name + ".srt") if wenkao else ""
    cmd = [PY, os.path.join(HERE, "make_srt.py"), work, "-o", out]
    if wenkao:
        cmd += ["--wenkao", wenkao, "--wenkao-out", wout]
...
def main():
    ...
    os.makedirs(WENKAO_DIR, exist_ok=True)
