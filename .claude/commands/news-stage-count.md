...
`news/smkul.csv` 是**節目目錄**，涵蓋全部有影片的集數（含還沒做的），所以它的列數是「總共有幾集」，不是「做了幾集」——拿它當分母，**表格最後一列放「smkul.csv 總集數」**（每月列數＋總和），跟其他階段的列放在一起，人一眼就看得出每個月做到幾成，不要只在文字裡帶過（使用者裁定 2026-09-23，覺得這樣有列出來很好）。
...
先出一張表：列＝階段（依管線順序）＋最後一列「smkul.csv 總集數」，
欄＝月份＋合計，儲存格＝集數。
