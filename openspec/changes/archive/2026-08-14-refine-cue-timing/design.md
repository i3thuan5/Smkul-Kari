...
歸類為「仍是左」「已是右」「空白」「不明」。邊界規則只有一條：
取「最後一張仍屬左側」與「第一張已屬右側」幀時間戳的**中點**——
兩張緊鄰時誤差對稱（±0.02s @25fps），中間隔著不明幀（交錯過渡）時
即該區段中點，同一條式。對接（A→B）的中點同時作為左 cue 的 end
與右 cue 的 start。

（規則修訂紀錄：35 集回頭精修的首批交付用的是舊規則「第一張右側幀
時間戳」，系統性偏晚 0–0.04s、仍在 0.05s 預算內；使用者決定不重跑，
中點制自下一次碰影片的批次起生效。）
