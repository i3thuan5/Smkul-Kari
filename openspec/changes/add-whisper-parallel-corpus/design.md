...
- **樣本數**：新聞 2021-01～10 各格的字幕數，0–10% 有 26,723、10–20% 有 20,040、20–30% 有 15,453、30–40% 有 9,904、40–50% 有 3,770，都比 385 多得多。每格 385 處，固定種子。
- **三個組**：每一處拿出三個組（只看這兩段，其他邊界一律不合併，以便隔離這一個變因）：
  - 合併版 G：兩段加上歸給它們的全部字幕；
  - 拆開版 A、B：各段加上歸給它的字幕，跨界字幕歸較多那邊。
  - 判定次數是 1,925 處 × 3 組 ＝ 5,775 次。
- **比較**：每組的分數 ＝ 族語詞數 ×（高 1、中 0.5、低 0）。合併版的分數大於 A、B 相加，算「合併較好」；小於算「拆開較好」；相等算「一樣」。用「可用詞數」比，是因為語料的價值在留下多少可用的詞，不在組數。
- **選值**：從最低的格往上看，最低的「合併較好比例 ＞ 拆開較好比例」、而且差距超出信賴區間的那一格，它的下緣就是門檻。沒有任何一格合併較好，就不合併。
...
- **成本**：5,775 次判定約 29 批。依 explore 實測推估，每批約等於 US$1.5～2 的牌價，合計約 US$50，算在 Max 20x 的額度裡。
...
| whisper-parallel-corpus | 同一種子抽到同一批；格內不足 385 處就全收並註明 | `mt/test_tuning.py` |
...
| whisper-parallel-corpus | 分數＝詞數 ×（高 1、中 0.5、低 0）；相鄰格差距在 Wilson 區間內 → 分不出；沒有一格合併較好 → 不合併 | `mt/test_tuning.py` |
...
- [±5 點誤差，相鄰格差不到約 7 點分不出] → 分不出時取較保守（較高）的門檻，因為合併錯了的損失（一整組被拖成低）比少併的損失大。
- [Max 額度] → 29 批分幾天跑，每批跑完就收件，可以續跑。
