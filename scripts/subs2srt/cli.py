...
from scripts.subs2srt import detect as detector
from scripts.subs2srt import ocr as recogniser
from scripts.subs2srt import sheets as contact
...
def stage_ocr(args):
    workdir = args.work
    manifest = assemble.read_manifest(workdir)
...
def stage_srt(args):
    workdir = args.work
    manifest = assemble.read_manifest(workdir)
    texts = assemble.load_transcripts(workdir)
...
def stage_import(args):
    imported, covered, total = assemble.import_tsv(
...
def stage_export_gt(args):
    ...
    verified = assemble.load_verified(workdir)
...
def stage_glossary(args):
    ...
    for cue in manifest["cues"]:
        ...
        for line in manifest["lines"]:
            ...
            if not value:
                continue
            for token in assemble.glossary_tokens(value):
