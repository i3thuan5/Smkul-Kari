...
## `text/`：上字文稿平行語料（跟上面全部無關，沒有影片）

原語會給的《開會了》上字文稿——001–045，跟這個 README 講的 068–164
影像側集號**零重疊**，那 41 集沒有影片，只有族語／華語平行語料。詳細
見 `Kari-SRT/aiyalaeho/text/README.md`；這裡只記怎麼跑：

```bash
scripts/aiyalaeho/text/fetch.sh                # SFTP 抓 243 個檔（9.2 MB）
python3 -m scripts.aiyalaeho.text.pairs        # 產出 1-句對.csv
```

兩支指令都是全量重跑，不是續傳——`fetch.sh` 每次先清掉本機暫存再重抓，
`pairs.py` 每次重新掃過暫存目錄。上游在 SFTP，這一層**離線重建不出來**，
「重跑一次、逐 byte 比對」就是這裡唯一能做的驗證。
