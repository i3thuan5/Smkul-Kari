...
def compare(slug, name):
    ...
    marker = os.path.join(plan_b, "from_rtf.json")
    if not (os.path.exists(marker) and os.path.isdir(plan_c)):
        return None

    rtf_cues = load(marker)
    ...
    for index in rtf_cues:
        ...
        if a == b:
            ...
            rows.append({
                ...
                "rtf": script.strip(), "vision": seen.strip(),
            })
    return {"episode": name, "slug": slug, "cues": len(rtf_cues),
...
def main():
    ...
    print("%-42s %6s %6s %6s %7s"
          % ("episode", "cues", "same", "diff", "agree"))
    ...
    if args.apply:
        for r in results:
            ...
            b_text = load(os.path.join(plan_b, "transcripts.json"))
            c_text = load(os.path.join(plan_c, "transcripts.json"))
            marker = load(os.path.join(plan_b, "from_rtf.json"))
            changed = 0
            for index in marker:
                key = str(index)
                if key in c_text:
