...
# There used to be a third rule here, on the column profile: the ink's right
# edge had to fall inside the compare window (x 1250-1790). It only ever
# refused episodes by mistake. The profile is summed over four minutes, so a
# newspaper clipping, a sign, leaves or a patterned tablecloth behind the
# band leaves a tail after the subtitle ends at x~1760, and the "right edge"
# came out at 1792-1863. All 9 episodes it stopped in 2021-01..04 were opened
# and looked at: every subtitle ended at x~1760, none reached past 1790.
# News subtitles are flush right to a fixed edge, so there is no layout left
# for it to catch. Removed (使用者裁定 2026-09-13).
...
def main():
    ...
    rows, frames = profile(args.video, probe, spec, args.start,
                           args.duration)
