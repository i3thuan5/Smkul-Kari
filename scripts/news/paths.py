...
# the store does (news/1-ocr/1-cues/): the number is the production order.
#
# The split exists because refine used to rewrite the coarse file in place.
# Two things went wrong with that, both silent:
#
#   * a refine killed midway left the coarse timeline damaged -- and that one
#     can only be regenerated by cutting the episode again, which means
#     downloading the 2 GB video again;
#   * two processes opening the same manifest (a refine still running while
#     something re-cut the same episode) meant last-writer-wins, with nothing
#     anywhere reporting it.
#
# With the stages separated the coarse file is written once and never touched
# again, so a failed refine costs only the refinement. 使用者裁定 2026-08-31.
COARSE_STAGE = "1-cues"
...
def cues_to_read(work):
    ...
    The flat `<work>/cues.json` is the pre-split layout, and it is last
    because it is a leftover, not a source of truth. It has to stay readable
    for now: the change lands while another line is midway through a
    seven-hour cutting batch that keeps producing the old layout, and a
    reader that only knew the new one would break that batch in half.
    `migrate_cues_layout` sweeps them, and then nothing reaches this arm.
    """
